...
#include "video_tools/visibility_control.h"
...
{
class Video2Image : public rclcpp::Node
{
public:
  VIDEO_TOOLS_PUBLIC
  explicit Video2Image(const rclcpp::NodeOptions & options)
  : Node("video2image", options),
    is_flipped_(false),
    publish_number_(1u)
  {
...
private:
  ...
  void initialize()
  {
    ...
    timer_ = this->create_wall_timer(
      ...
      std::bind(&Video2Image::timerCallback, this));
  }
  ...
  bool help(const std::vector<std::string> args)
  {
    ...
    {
      std::stringstream ss;
      ss << "Usage: video2image [-h] [--ros-args [-p param:=value] ...]" << std::endl;
      ss << "Publish images from a video stream." << std::endl;
      ss << "Example: ros2 run video_tools video2image --ros-args -p reliability:=best_effort";
      ...
      ss << std::endl << std::endl;
      ss << "  video_path\t\tpath of the video . Default value is 'video_frame'";
      ss << std::endl << std::endl;
  ...
  void parse_parameters()
  {
    ...
    video_path_ = this->declare_parameter("video_path", "sample_720p.mp4");

	std::cout << "video_path" << video_path_ << std::endl;
  }
...
RCLCPP_COMPONENTS_REGISTER_NODE(video_tools::Video2Image)
